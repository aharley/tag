...
class AntLegDataset(PointDataset):
    def __init__(self,
                 dataset_location='/orion/group/antleg', 
                 use_augs=False,
        ...
        print('loading antleg dataset...')
        
        files = glob.glob('%s/AllTracks*' % self.dataset_location)
        files = sorted(files)

        raw_data = pd.read_pickle(files[0])
        # print(raw_data)
        # exit()
        # print('npzs', npzs)
            ...
            for kp_id in range(N):
                ...
                if pick_id >= 0:
                    for stride in strides:
                        ...
                        if fullseq and len(full_idx) < S: continue
                        elif len(full_idx) < 8: continue
